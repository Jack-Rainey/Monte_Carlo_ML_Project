...
save/show levels (enforced by `tests/test_verbosity_gate.py`).
...
to no-hidden-defaults, and `Config` never carries it (CLAUDE.md
...
stage function — never a module global.

It travels inside a frozen `RunContext`, and stage functions are
...
| 4 | diagnostics | `renders/qc_record.csv` (every render QC criterion measured, pass or fail) | *(no sites today — reserved)* |
...
Two render-stage QC artifacts, on opposite sides of the gate. `qc_failures.csv`
is **canonical**: it is the evidence behind the stage's refusal, and a raise
whose evidence file is suppressed at the default save level is not a reportable
result. `qc_record.csv` — the same criteria measured for every scene, pass or
fail — is the level-4 observability artifact. `renders/<id>/meta.json` is
likewise canonical: it is the sole record of how an expensive dataset was made
(installed simulator version, both ray budgets, declared speed of sound,
ambisonic convention), and gating it meant the default `save=1` run produced a
dataset nobody could later characterize.

Outside the ladder entirely: **warnings and errors always emit, to stderr,
at every show level** (a suppressed fatal error is never acceptable).
...
a default. Quiet levels still satisfy design_spec §9's git-SHA bundle
...
authoring/preview front-end (a deliberate forward-looking
seam, not scope creep; do not strip). Nothing emits it today. Its TTY guard
already exists: live `visual` output requires `show>=5` **and** an
...
`amcd.runtime.emit(verbosity, category, msg)` (no scattered
...
| render | rendered/reused/refused summary (progress); QC pass count (metrics) | `renders/qc_record.csv` (diagnostics); IR pair, `renders/<id>/meta.json`, `renders/<id>/paths_{low,high}.parquet` and `renders/qc_failures.csv` are canonical |
...
Classification rule for new artifacts: if anything downstream — a
