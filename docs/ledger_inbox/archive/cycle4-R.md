...
`max_t60_over_ir_duration_frac: 0.0` gate are all computed for the wrong room, and
...
1. **AC-40** (blocker, effective absorption `1-sqrt(1-alpha)`): one shoebox at
   ...
   `ir_duration: 4.25` choice and the `max_t60_over_ir_duration_frac: 0.0` gate
