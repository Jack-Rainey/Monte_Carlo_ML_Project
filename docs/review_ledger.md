...
| F-37 | falsifier | major | OPEN | src/amcd/training/infer.py; src/amcd/evaluation/evaluator.py | The F-25 residue pattern one stage downstream: `predictions/` was never cleared and `run_eval` GLOBS `predictions/*_pred.pt`, assigning each to the split from the CURRENT splits.json. After a re-run that moved a scene out of a test split, the OLD model's prediction survived and was scored. REPRODUCED this session: 13 predictions on disk where 11 were produced, 2 stale ones scored under `train`, 10 metric rows in metrics.parquet. | FIX APPLIED, awaiting re-review: infer clears `*_pred.pt` + `*_decoded_ir.npy` before writing; eval raises if a prediction's split is not a test split (infer only ever writes test-split predictions, so such a file is definitionally residue). Probe re-run: 11 predictions, no `train` among scored splits, 0 leaked rows. Tests: TestPredictionResidue. |
| F-38 | falsifier | major | OPEN | src/amcd/data/preprocess.py; src/amcd/config.py | The rmtree exclusion `stale.name != "carrier"` created an unreserved magic name — the same defect class as F-28, in the same commit that fixed it. A split named `carrier` would be permanently exempt from the stale-split sweep, reinstating the F-25 leak for itself, and would collide with the raw carrier `.npy` files in that directory. | FIX APPLIED, awaiting re-review: `RESERVED_SPLIT_NAMES = (ID_POOL_TAG, "carrier")` rejected at load. NOT fixed: carrier `.npy` orphans after a shrink are still unpruned — inert for reads (looked up by scene_id, never globbed) but silent residue; tracked here. |
| F-39 | falsifier | minor | OPEN | src/amcd/data/dataset.py | The orphan backstop globbed only `*_low.pt` while its docstring claimed "any tensor not in the manifest", so an orphan `_high` with no `_low` partner was invisible — in the consumer-side guard for the F-25 blocker. | FIX APPLIED, awaiting re-review: globs `*.pt` and strips the suffix. Test: TestOrphanHighTensor. |
| F-40 | falsifier | minor | OPEN | src/amcd/data/preprocess.py; src/amcd/data/dataset.py | A declared split receiving zero scenes had no directory, so EnergyDataset raised `FileNotFoundError: Split directory not found` — pointing at the filesystem rather than at the empty split F-30 now reports as 0. Two vocabularies for one condition; a hard crash for train/valid. | FIX APPLIED, awaiting re-review: preprocess mkdirs every declared split dir, so the manifest-keyed error is the single message. |
| F-41 | falsifier | minor | OPEN | src/amcd/pipeline.py | `_recorded_fingerprint` returned None for three distinct conditions, one of which is "upstream legitimately declares no fingerprint" — so the first chain crossing a `None`-fingerprint stage would report "has not completed" for a stage that did. Unreachable today; RD-41 exists to wire the remaining seven stages, which is exactly what would trigger it. | FIX APPLIED, awaiting re-review: chaining to a stage with no declared fingerprint now raises its own explicit error naming the cause. |
| F-42 | falsifier | minor | OPEN | src/amcd/scenes/generator.py (_room_acoustics) | `drr_db` divides by d² with no guard on `distance` (alpha IS guarded), so a degenerate pair would write `+inf` into the canonical placement_report.json with only a numpy warning. Reachable because base.yaml's `interior_random` declares `distance_range: null`. | FIX APPLIED, awaiting re-review: `_room_acoustics` raises on `distance <= 0` rather than clamping, so a degenerate scene cannot be reported as if it were real. |
| F-43 | falsifier | minor | OPEN | src/amcd/simulators/dry_run.py | `distance` was silently clipped to 0.3 m BEFORE the delay was computed, so for any closer pair the scaffold emitted the onset of a 0.3 m path — contradicting the speed of sound the AC-11 fix had just made it declare into canonical provenance. Also the same degenerate-separation family as AC-13. | FIX APPLIED, awaiting re-review: the clip is now a loud rejection against a CONFIG-declared `min_source_receiver_distance_m` (0.3 m), documented as matching the old clip and as distinct from the real backend's source_radius + listener_radius floor. This is the AC-13 guard for the dry_run path; the gsound-side guard lands with Step 3. |
| AC-09 | acoustics-reviewer | major | OPEN | src/amcd/scenes/generator.py (placement_report.json) vs RD-29 | The artifact built to quantify RD-29 records source-receiver DISTANCE, but the quantity RD-29 is about is DRR → C50/D50/EDT, which depends on d/r_c (r_c = 0.057·sqrt(V/T60)), not d. Measured over the real 720-scene RI config: test_material_shift median d = 3.64 m vs train 3.44 m (indistinguishable) yet median DRR +4.9 vs −5.8 dB — an ~11 dB difference invisible in the recorded statistic, because absorption moved r_c from 1.53 to 3.15 m. Conversely test_geometry_shift differs 1.3 m in median distance but only 0.7 dB in DRR. As recorded, the artifact cannot support the E1 disclosure it exists for. | Add per-split volume, surface, absorption, Sabine AND Eyring T60, r_c, d/r_c, estimated DRR = 10log10(R/(16πd²)), and the direct-path delay range d/c (ms) that Step 4's onset QC is read against. Distance stays but is not the headline. FIX APPLIED: placement_report.json now records volume, Sabine AND Eyring T60, critical distance, d/r_c and diffuse-field DRR per split. Verified on the real RI config: train median DRR -5.8 dB vs test_material_shift +4.9 dB at near-identical median distance (3.44 vs 3.64 m) — the ~11 dB the distance-only artifact could not show. E1 disclosure obligation remains OPEN. |
| AC-10 | acoustics-reviewer | major | OPEN | src/amcd/scenes/generator.py (`rcv_hi = lo[i] + corner_frac*(hi[i]-lo[i])` for i in range(3)) | `corner_frac` is applied to ALL THREE axes including z. With research_i.yaml's height_range [1.2,1.8] and corner_frac 0.2, near_corner receivers are confined to z ∈ [1.200, 1.313] (verified on the 30 generated scenes: rcv_z mean 1.242 vs src_z 1.520; other splits ≈1.50). Once a height_range is declared z is an ergonomic band, not a room boundary — there is no corner to be near in z, so the bias buys no boundary proximity while silently narrowing a reported robustness split's receiver-height distribution ~5×. placement_report's POOLED height stat hides it. | Apply corner_frac to the horizontal axes only; record source and receiver heights SEPARATELY. Known-answer test: near_corner with a height_range must leave receiver z spanning the full band while x,y are confined. FIX APPLIED: corner_frac applied to the horizontal axes only; source and receiver heights recorded separately. Receiver z now spans the declared band (was [1.201, 1.313]); known-answer test added. |
| AC-11 | acoustics-reviewer | major | OPEN | src/amcd/simulators/dry_run.py:16,71 vs src/amcd/simulators/base.py (REQUIRED_PROVENANCE_KEYS) | The new provenance contract defines `speed_of_sound_m_s` as "the speed the backend actually used"; dry_run declares 343.0 but applies NO propagation delay — probe: d = 1.0/5.0/9.8 m all give onset = 0 samples (expected 140/700/1371). Effective c is infinite, so the first value the contract stamps into canonical meta is false, and the scaffold can never exercise onset alignment (AC-02) or the geometric onset check the real backend WILL trigger. Constant is also a Python literal. NOTE: the 343-vs-344 m/s question that prompted RD-19 is harmless — 0.086 ms at the 9.84 m max separation vs a 2.0 ms tolerance, and the RI onset QC is a paired within-backend check that cancels exactly. The risk is the zero, not the 1 m/s. | Give dry_run a real d/c delay driven by the declared constant (preferred — exercises the onset path before Rosetta) and move the constant into configs/simulators/dry_run.yaml. Confirming test: onset(render(d)) == round(d/c·fs) ± 1 sample for three distances; then re-run D0a/D0b on delayed IRs to prove onset alignment neutralizes delay. FIX APPLIED: dry_run places the whole response (direct AND tail) at d/c from a CONFIG-declared `speed_of_sound_m_s`; test asserts onset == round(d/c*fs) +/-1 sample at 1.0/5.0/9.8 m, which previously returned 0 for all three. |
| AC-12 | acoustics-reviewer | minor | OPEN | configs/simulators/gsound_sir.yaml (`frequency_points[0] = 88.4`) | Derivation and six of seven numbers CONFIRMED against upstream (Context.cpp:8 centres {63…8000}; gsFrequencyBands.cpp:83-88 geometric-mean crossovers; binding.cpp:304 len == n_bands−1; :334 Linkwitz-Riley CrossoverFilter). But sqrt(63×125) = 88.7412, not 88.4 — 88.4 is sqrt(62.5×125), the nominal base-two centre. The inline comment "sqrt(63*125)=88.4" is arithmetically false. Error −0.384% on that one crossover; all others ≤0.013%. Independently re-verified this session. | Set frequency_points[0] = 88.7412 and fix the comment. Physically negligible at iso_eval_freqs 500/1000, but the point of the row is exact agreement with the ray generator's own band definition. FIX APPLIED: frequency_points[0] = 88.7412 (sqrt(63*125)); the false 'sqrt(63*125)=88.4' comment corrected, all seven edges given to 4 dp. Arithmetic independently re-verified. |
| AC-13 | acoustics-reviewer | minor | OPEN | configs/base.yaml (interior_random distance_range: null) vs configs/simulators/gsound_sir.yaml (source_radius/listener_radius 0.1) | With no distance constraint base.yaml admits separations below source_radius + listener_radius = 0.2 m: P(d<0.2) = 0.049%/scene (~0.34 in a 700-scene set), P(d<0.5) = 0.70% (~5 scenes), min over 5e5 draws 0.031 m. Overlapping source/listener spheres are geometrically degenerate for gsound and 1/d² blows up. dry_run masks it with a hardcoded `np.clip(distance, 0.3, None)`, so the scaffold cannot surface it. Unexercised today (real render raises); valid input tomorrow. | Guard, don't remove generality: declare a minimum separation in base.yaml (≥ r_src + r_lis, upper bound still null) or have the render stage reject d < r_src + r_lis naming the scene. Known-answer test: d = 0.15 m must fail loudly. |
| AC-14 | acoustics-reviewer | minor | OPEN | src/amcd/scenes/generator.py (single `acceptance_rate`) | One scalar conflates two rejection mechanisms of OPPOSITE acoustic sign. Measured (2e5 draws, RI ranges): shoebox rejects 7.64% below the 1.0 m minimum vs 0.93% above the 10 m cap; corridor 9.04% below vs 12.65% above. The constraint strips the high-DRR tail in shoeboxes and the low-DRR tail in corridors. Also unrecorded: 49.4% of RI shoeboxes cannot reach 10 m separation at all (smallest 4×3 m: max 3.655 m) vs 17.4% of corridors — the asymmetry the E1 report must state. | Split into rejected_below_min / rejected_above_max per split; record the fraction of sampled rooms whose max achievable separation < distance_range[1]. FIX APPLIED: rejections split into rejected_below_min / rejected_above_max, plus rooms_that_cannot_reach_max_distance, per split. Measured on RI: train 38 below vs 4 above, and 237 of 500 shoeboxes cannot reach the 10 m cap. |
| AC-15 | acoustics-reviewer | minor | OPEN | src/amcd/simulators/base.py (`e.g. "acn_sn3d"`) — ANSWERS RD-25 | Upstream is ACN ordering with **N3D**, not SN3D: auralizer/src/cpp/binding.cpp:18 ("normalization constant K(l,m) for N3D") and :43 ("real-valued Spherical Harmonics (N3D/ACN ordering)"); (order+1)² enforced at :62,301 (16 at order 3, matching Step 0's (16, 92859)). The docstring example is the value most likely to be copied into gsound provenance at Step 3 and would be wrong by the SN3D↔N3D sqrt(2l+1) per-degree scaling. | Stamp `ambisonic_convention: "acn_n3d"` at Step 3; fix the docstring example. Blast radius nil today (every live scalar metric uses channel 0, where N3D and SN3D agree exactly, K(0,0)=1), load-bearing once evaluation/spatial.py is filled in. FIX APPLIED (docstring half): REQUIRED_PROVENANCE_KEYS documents acn_n3d with the binding.cpp evidence. Stamping it for gsound_sir is Step 3; row stays OPEN until then. |
| AC-16 | acoustics-reviewer | minor | OPEN | src/amcd/representations/spectrogram.py vs configs/simulators/gsound_sir.yaml | The representation resolves 27 third-octave bands over a simulation with only 8 octave bands of spectral freedom: exactly 3 third-octaves per simulated band except the TOP band [5656.9, 24000] Hz — 2.08 octaves wide at 48 kHz — which backs 6 third-octaves with 8 kHz air absorption throughout; three bands (24.8/49.6/78.7 Hz) sit inside the DC-88.7 Hz band. ISO metrics unaffected; any broadband energy-residual quantity has 27 nominally independent dimensions backed by 8, ~22% carrying no independent simulated content. | Not a bug — record it. The E2 energy-residual claim must not assert third-octave spectral fidelity the simulator does not produce; consider a band-limited (88.7 Hz-5.66 kHz) residual alongside the broadband one. |
