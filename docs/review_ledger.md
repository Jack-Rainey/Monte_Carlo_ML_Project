...
| RD-28 | research-director | major | DEFERRED (implementation) / OPEN (disclosure) | plan Step 1.5; src/amcd/simulators/base.py:16 (`material_absorption: float`) vs research_I_paper.md l.512 | RI's material shift is "ceiling_absorptive AND asymmetric_walls"; `asymmetric_walls` needs per-surface absorption, which a scalar `material_absorption` (and `createbox(absorp)`, scalar-or-per-band) cannot express. Dropping half the shift changes its MAGNITUDE, so `test_material_shift` is not comparable to RI's number — same defect class as RD-26/RD-27, hence major not minor. | Implementation DEFERRED to a per-surface-materials gate. Disclosure is NOT deferrable: OPEN at E1. **SIGN CORRECTED (AC-20, 2026-08-10)** — the earlier text here said the shift is "half of RI's", which is wrong in direction: the scalar α is applied to ALL SIX SURFACES, giving Sabine T60 0.117/0.161/0.209 s vs an area-weighted ceiling-only reading's 0.42/0.51/0.65 s, i.e. **3-4x STRONGER, not weaker**. The E1 report states it as measured. **CONTINGENCY (user, 2026-08-10):** per-surface materials stay deferred so implementation can scale, but gsound's `createbox(absorp)` takes scalar-or-per-band and NOT per-surface, so if per-surface proves unreachable through that API this decision reopens — and that constraint is itself an argument for an eventual custom simulator rather than a GSound-SIR workaround. |
...
### research-director rows from the pass-3 remediation PLAN review (2026-08-10)

Raised on the plan BEFORE implementation, per CLAUDE.md. Those already discharged
by the commits below are marked so; the rest are OPEN.

| ID | agent | sev | status | anchor | finding | resolution |
|----|-------|-----|--------|--------|---------|------------|
| RD-42 | research-director | major | OPEN | pass-3 remediation plan Step 7 vs RD-33's lift condition | The cycle was titled "lift the RD-33 render gate" but cannot: RD-33's condition is a clean pass, CLAUDE.md defines clean as zero OPEN rows, and this cycle fixes ~7 of 47. Either the gate never lifts or someone declares it lifted with ~30 OPEN rows — the dishonest-completion failure the Reporting rule forbids. | **USER DECISION (2026-08-10):** do NOT mass-triage to DEFERRED; the remaining rows stay OPEN and are addressed next pass. So **RD-33 does not lift this cycle** and the plan was retitled. Only rows genuinely belonging to a later gate get DEFERRED. |
| RD-43 | research-director | major | OPEN | src/amcd/evaluation/room_acoustic.py (`_shared_truncation_per_band`) | The plan's "min over all legs" would let a MODEL OUTPUT set the integration window used to measure its own ground truth: a degenerate pred with an early-rising floor shortens the window for high and low, compressing the legs together and shrinking abs(pred-high) — a worse model scoring better by corrupting the measurement of its target. Not the AC-08 analogue the plan claimed: band intersection drops a band as a visible NaN; truncation-min silently redefines the window of legs that were fine. | FIX APPLIED (f3c3543), awaiting re-review: the window derives from the PHYSICAL legs only and is applied to pred. Test `test_prediction_cannot_set_the_integration_window` swaps a healthy pred for a degenerate one and asserts the window and both physical legs are unchanged. |
| RD-44 | research-director | major | OPEN | src/amcd/evaluation/evaluator.py; docs/design_spec.md §3 | Option (a) removes the differential bias but leaves a COMMON-MODE absolute bias set by the noisiest leg — so high's measured T30/EDT/C50 becomes a function of the LOW leg's ray budget (contradicting RD-17's "converged reference" framing), an E4 metric-vs-ray-count curve is confounded by its own estimator, and absolutes are no longer comparable to RI or the literature. | FIX APPLIED (f3c3543), awaiting re-review: window + limiting leg recorded for every scored scene in `metrics/iso_integration_windows.json`; design_spec §3 states absolutes are paired-comparison quantities only. **STILL OPEN:** the E4-gated DEFERRED row for Lundeby extrapolated-tail compensation (the budget-independent estimator the sweep needs) — see RD-55. |
| RD-45 | research-director | major | OPEN | configs/base.yaml (`near_corner` distance_range: null) | Step 4 floored only `interior_random`; `near_corner` also declares `distance_range: null` and is the regime behind `test_placement_shift` (30 scenes, ~5-6 % abort probability per base run). The plan's own pass condition — "base.yaml must not be able to emit d below the declared floor" — would have FAILED as written, and the verification would have been reported green against a false claim. | NOT YET IMPLEMENTED (Step 4 not started). Floor EVERY placement regime, and assert at config load / gen-scenes that each regime's lower bound ≥ the active simulator's declared floor, keeping the render pre-flight as the backstop. |
| RD-46 | research-director | major | OPEN | configs/research_i.yaml `ceiling_absorptive` | Step 6 corrected the WORDING of a disclosure whose underlying split may be scientifically void: α on six surfaces is near-anechoic (AC-20), the diffuse-field premise has failed for 100 % of it (AC-21), and its EDT approaches filter resolution (AC-23) — 40 scenes × 2 legs of emulated render returning estimator artifacts, under a per-shift breakdown that IS the research result. The α range is a v3 invention, not RI-pinned, so it is free now and expensive later. | **USER DECISION (2026-08-10):** option (b) — keep the range, keep the split for the PAIRED comparison, suppress absolute ISO values below `metric_min_measurable_t60_s`, and mark the split diagnostic-only in research_i.yaml (done, Step 6). RD-28 carries the per-surface-materials contingency. **CAVEAT:** the AC-23 threshold is 0.05 s from measurement, and test_material_shift's Eyring median is 0.059 s — so most of the split is NOT in fact suppressed. The diagnostic-only framing therefore rests on the DISCLOSURE, not on numerical suppression. Revisit if AC-23's magnitude is re-established. |
| RD-47 | research-director | major | OPEN | src/amcd/simulators/dry_run.py | AC-18's deferral was defensible on gating grounds but inefficient: a one-line change that re-datasets every dry_run artifact, deferred into a second invalidation of the very artifacts Step 5 produces as gate evidence. | **USER DECISION:** pulled in as Step 4.5. FIX APPLIED (2433b03), awaiting re-review. |
| RD-48 | research-director | minor | OPEN | src/amcd/config.py:375-383 (`distance_range`) | A nullable ELEMENT is a new sub-convention, not the existing whole-range null; widening the type admits `[null, hi]` (meaningful) and `[null, null]` (a second spelling of `null`) — two ways to say one thing. | NOT YET IMPLEMENTED (Step 4). Reject `[null, null]` naming `distance_range: null` as the spelling; keep `lo < hi` when both bounds present; document all three legal spellings in the field's `#:` comment. |
| RD-49 | research-director | minor | OPEN | src/amcd/simulators/base.py | Category error with a real consequence: `REQUIRED_PROVENANCE_KEYS` are validated on an `IRResult`, i.e. AFTER a render, but the minimum separation is needed BEFORE one. As an optional attribute, raytracer #2 omits it and pre-flight silently degrades to a 0.0 floor — the silent-contract failure RD-31 exists to close. | NOT YET IMPLEMENTED (Step 4). Declare it a required `Simulator` interface member checked at `build_simulator` time. Test: a stub simulator missing the declaration fails loudly at build, not at render. |
| RD-50 | research-director | minor | OPEN | src/amcd/scenes/generator.py:276-284 vs RD-36 | "RD-36's baseline is superseded" is stronger than the facts and discards a cheap, sharper verification: each scene draws from its own `scene_rng`, so a rejection perturbs only that scene (~1 in 600 at the measured 0.186 % rate). | NOT YET IMPLEMENTED (Step 4). After the change, re-run base+dry_run and report "N of 600 scenes differ, and all N violated the declared floor" rather than abandoning bit-identity. |
| RD-51 | research-director | minor | OPEN | configs/ (new `overlays/` dir) vs README:61-69 | `configs/` gains a SECOND subdirectory semantics: `models/`, `representations/`, `simulators/` hold plugin params selected BY NAME and never passed to `-c`, whereas `overlays/` holds files that ARE. It also creates a third overlapping dry-run entry point alongside `configs/dry_run.yaml`, which F-51's error message must then choose between. | NOT YET IMPLEMENTED (Step 5). README states both semantics explicitly; ONE canonical dry-run invocation is named and the others compose it. |
| RD-52 | research-director | minor | OPEN | plan Step 6 probe (RD-17) | AC-17's fix is exercised only by a synthetic known-answer test, because the defect is inert under dry_run by construction. Its first exercise on real data would otherwise be the expensive emulated render it was written to protect. | Add to the RD-17 ray-budget probe's artifact: per-leg vs shared truncation indices on REAL IRs, plus T30/C50 computed both ways. Costs nothing extra — the probe already renders those legs and runs the ISO path. Gate: with RD-17. |
| RD-53 | research-director | minor | OPEN | src/amcd/config.py (`REQUIRED_ROLE_COUNTS`) | The cardinality rule hardcodes single-holdout validation into config validation; k-fold / repeated holdout under the roadmap's deeper hyperparameter search would need multiple `valid` splits. | FIX APPLIED (1b9b194), awaiting re-review: expressed as the declared `REQUIRED_ROLE_COUNTS` mapping with the roadmap item named in its comment, and consumed through one lookup helper (`split_names_with_role` / `the_split_with_role`). |
| RD-54 | research-director | minor | OPEN | pass-3 commits vs RD-41 | Steps 1 and 3 silently invalidate artifacts where Step 4 declares that it does. Step 1 changes the reported-metric path, so every existing `metrics.parquet`, `stats/ci_table.csv` and `diagnostics/d0b_oracle.json` is superseded — and with seven stages carrying no fingerprint (RD-41), a cached run_dir serves OLD metrics under NEW code. Step 3 changes report ordering alphabetical → declaration order. | PARTIALLY APPLIED: both invalidations are stated in their commit messages. **STILL OPEN:** promoting RD-41's `eval`/`stats` fingerprints out of DEFERRED, which is exactly the "metric definition changed under a cached run" hazard that row describes. |
| RD-55 | builder (follow-on to RD-44) | minor | DEFERRED | src/amcd/evaluation/room_acoustic.py (gate: E4) | The shared window is set by the noisiest leg, so reported ISO absolutes remain a function of the low-ray budget. An E4 "metric vs ray count" curve therefore mixes genuine convergence with a window that shrinks as the budget drops. | Not this gate: paired improvement — everything E1/E2 report — is unaffected. Implement Lundeby extrapolated-tail compensation (ISO 3382-1 Annex) when E4 first needs budget-independent absolutes. |
...
**2026-08-10 (pass-3 REMEDIATION, cycle 1 — INCOMPLETE). 5 of 8 plan steps done.
RD-33 HAS NOT LIFTED. No real gsound render.**

Plan: `~/.claude/plans/floating-nibbling-key.md` (approved after a
research-director review that raised RD-42..RD-54 — 6 major, 7 minor, all folded
in). Suite **217 passing** (was 208).

DONE and committed:
- `f3c3543` **AC-17 + RD-43 + RD-44 + AC-23** — cross-leg shared Schroeder window,
  derived from the PHYSICAL legs only; D0b probe moved onto it (defect was not in
  the reviewer's anchor); `metric_min_measurable_t60_s`;
  `metrics/iso_integration_windows.json`; design_spec §3 disclosure.
- `1b9b194` **F-44 + RD-53** — `SPLIT_ROLES` + `REQUIRED_ROLE_COUNTS` at config load.
- `0b9b7f5` **F-45** — stats/report/probe/infer enumerate DECLARED splits.
- `2433b03` **AC-18 + AC-24** — unbiased dry_run tail; shared `amcd.acoustics`.
- **AC-20** (Step 6) — material-shift disclosure corrected in `configs/research_i.yaml`
  + RD-28's matching sign error. Uncommitted at time of writing if `git status` is dirty.

**NOT DONE — resume here, in this order:**
1. **Step 4 (F-48 / AC-13 / RD-45 / RD-48 / RD-49 / RD-50)** — minimum separation.
   Floor **every** placement regime (`near_corner` too — RD-45), `[lo, null]`
   schema rejecting `[null, null]` (RD-48), min separation as a required
   `Simulator` interface member checked at `build_simulator` (RD-49, NOT a
   provenance key), render pre-flight over all scenes, and the RD-50 re-baselining
   claim instead of abandoning RD-36.
2. **Step 5 (F-52 / RD-51)** — `configs/overlays/`, README's two subdirectory
   semantics, one canonical dry-run invocation. Then the RD-33 evidence: full
   `base+research_i` through **gen-scenes only** (720 scenes + membership table),
   and `base+research_i+overlays` through all nine stages.
3. **Step 7** — pass-4 reviewers: `falsifier`, `acoustics-reviewer`,
   `research-director`, `readability-reviewer` (the last has NEVER run on Step
   1A/1.5). Then triage.

**Three things a resumer must not miss:**
- **AC-23 did not reproduce.** It claimed EDT reads +89 % at T60 = 0.06 s; measured
  here it is +8.3 %, and the residual EDT error is non-monotonic across 0.03-0.5 s
  (realization variance, not a measurability floor). Physically the octave filter
  at 500 Hz is ~353 Hz wide, so its ringing lasts ~3 ms and cannot dominate a 60 ms
  decay. `metric_min_measurable_t60_s = 0.05` is set from the measurement, so it
  suppresses far less than AC-23 implied — which is why RD-46's diagnostic-only
  framing rests on the disclosure, not on suppression. The row stays OPEN for the
  acoustics-reviewer to supply its exact probe or withdraw the magnitude.
- **AC-18 was NOT fixed as written.** Its proposed `decay * (1 + noise*σ)` makes the
  tail strictly positive (verified: zero sign changes) — an envelope, not a
  pressure signal — which would carry almost no energy through the 500/1000 Hz
  octave filters. Implemented instead as converged-response + estimation-noise.
- **D0a's verdict flipped and that is expected.** With the unbiased tail D0a now
  reports a 0.05 dB gap ("denoising unlikely to help") where the bias gave it 16 dB
  of apparent headroom — so D0a's old "substantial gap" was an artifact too, which
  AC-18 did not name. Neither magnitude is calibrated to real ray tracing. RD-07
  stands in full: no D0a/D0b verdict from this scaffold is a scientific result.
