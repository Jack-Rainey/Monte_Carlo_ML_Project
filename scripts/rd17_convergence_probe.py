...
def _iso(config: Config, ir: np.ndarray) -> dict:
    ...
    per_band = channel_per_band_metrics(
        ir, sample_rate=config.sample_rate, iso_eval_freqs=config.iso_eval_freqs,
...
def main() -> int:
    ...
    for scene in _probe_scenes(config, args.scenes):
        ...
        for leg, budget in (("low", config.low_ray_budget),
                            ("high", config.high_ray_budget)):
            t0 = time.time()
        ...
        # The verdict: does the LOW leg reproduce the HIGH one inside tolerance?
        # Framed against the high leg because that is the one being trusted.
        verdict = {}
        for fc in row["legs"]["high"]["iso"]:
            hi, lo = row["legs"]["high"]["iso"][fc], row["legs"]["low"]["iso"][fc]
            band = {}
    ...
    total = sum(r["legs"][l]["wall_clock_s"] for r in results for l in ("low", "high"))
